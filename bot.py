...
async def play_song(interaction: discord.Interaction, query: str):
    ...
    if not interaction.user.voice:
        await interaction.followup.send("🎤 先進語音頻道", ephemeral=True)
        return
    ...
    if not vc:
        await interaction.followup.send("❌ 無法連線語音（4006）", ephemeral=True)
        return
    ...
    try:
        ...
        await interaction.channel.send("❌ YouTube 擋住或找不到歌曲")
        return
...
class MusicView(discord.ui.View):
    ...
    async def leave(self, interaction, button):
        ...
        if vc:
            await vc.disconnect()
        await interaction.response.send_message("👋 離開語音", ephemeral=True)
...
class MainView(discord.ui.View):
    ...
    async def couple(self, interaction, button):
        await interaction.response.edit_message("💞 情侶系統", view=CoupleView())

    @discord.ui.button(label="🎁 驚喜", style=discord.ButtonStyle.success)
    async def gift(self, interaction, button):
        await interaction.response.edit_message("🎁 驚喜", view=GiftView())

    @discord.ui.button(label="💬 日常", style=discord.ButtonStyle.secondary)
    async def chat(self, interaction, button):
        await interaction.response.edit_message("💬 日常", view=ChatView())
